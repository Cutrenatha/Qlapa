...
def db_product_query(types):
    prods = Product.query.filter(Product.type.in_(types), Product.status == "active").limit(3).all()
    return [p.to_dict() for p in prods]

import ai_engine
ai_engine.register_db_callback(db_product_query)
...
def create_product():
    ...
    data = request.get_json(force=True)
    required = ["name", "category", "type", "price", "stock"]
    if not all(data.get(f) is not None for f in required):
        return jsonify({"error": "Nama, kategori, jenis, harga, dan stok wajib diisi"}), 400
...
def create_order():
    ...
    for seller_id, pairs in grouped.items():
        ...
        order = Order(
            ...
            shipping_cost=shipping_cost,
            shipping_address=shipping_address if shipping_method == "kirim" else "Ambil Sendiri (Pick Up)",
            status="dikemas" if payment_method == "cod" else "belum_bayar",
            payment_status="pending",
            midtrans_tx_id=midtrans_tx_id
        )
        db.session.add(order)
...
def update_order_status(order_id):
    ...
    new_status = request.get_json(force=True).get("status")
    seller_moves = {"dikemas", "dikirim", "dibatalkan"}
    ...
    if user.id == order.seller_id and new_status in seller_moves:
        ...
        # Buyer can only cancel if it's unpaid (belum_bayar)
        if new_status == "dibatalkan" and order.status != "belum_bayar" and order.status != "dibatalkan":
            return jsonify({"error": "Tidak bisa membatalkan pesanan yang sudah dibayar"}), 400
...
def ai_chat():
    ...
    # Extract products list from DB matching user's inquiry
    from ai_engine import detect_intent, extract_product_types, fetch_products_by_types, BAHAN_BAKU_LIST, DOWNSTREAM_MAP, ALLOWED_TYPES, PRODUK_OLAHAN_LIST
    ...
    if intent in ["product_recommendation", "product_search", "product_utilization"]:
        ...
        products_list = fetch_products_by_types(search_types)
...
def ai_recommendation():
    ...
    # Fetch recommendation description
    rec_text = get_recommendation(category)
    
    # Pull products matching the category (and downstream if it is a raw material)
    from ai_engine import fetch_products_by_types, BAHAN_BAKU_LIST, DOWNSTREAM_MAP, ALLOWED_TYPES, PRODUK_OLAHAN_LIST
    search_types = [category]
    if category in BAHAN_BAKU_LIST:
        all_types = BAHAN_BAKU_LIST + PRODUK_OLAHAN_LIST
        downstream = DOWNSTREAM_MAP.get(category, {}).get("produk", [])
        for ds in downstream:
            for canonical in all_types:
                if ds.lower() == canonical.lower() and canonical not in search_types:
                    search_types.append(canonical)
    ...
    products_list = fetch_products_by_types(search_types)
    return jsonify({"recommendation": rec_text, "products": products_list})
